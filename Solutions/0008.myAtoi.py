...
输入: "-91283472332"
输出: -2147483648
原因: 数字 "-91283472332" 超过 32 位有符号整数范围。因此返回INT_MIN。
```

## 题解
按题意模拟运算过程，注意正负号和数值越界的问题。
"""
...
class Solution:
    def myAtoi(self, s: str) -> int:
        ...
        while i < len(s) and s[i] == ' ':
            i += 1
        if i < len(s) and s[i] in symbols:
            if s[i] == '-':
                flag = False
            i += 1
        while i < len(s) and s[i] in nums:
            if flag and res > (INT_MAX - int(s[i])) // 10:
                return INT_MAX
            elif not flag and -res <= (INT_MIN + int(s[i])) // 10:
                return INT_MIN
            else:
                res *= 10
                res += int(s[i])
